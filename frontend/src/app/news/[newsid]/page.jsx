    ...
    <div className="max-w-[1480px] mx-auto px-5 sm:px-8">
      <div className="max-w-screen-md mx-auto">
        <div className="flex flex-wrap gap-3 items-center text-[15px]">
          <div className="flex flex-wrap gap-3">
            <Link
              ...
              href={`/category/${news.categoryName ? news.categoryName.toLowerCase() : ''}`}
            >
              {news.tags}
            </Link>
          </div>
        ...
        <div className="flex gap-2 items-center mt-4">
          <div className="flex">
            <Link className="flex -ml-3 first:ml-0 first:z-10 hover:z-20 " href={`/author/${news.authorName ? news.authorName.toLowerCase() : ''}`}>
              <div
          ...
          <div>
            <Link className="text-sm font-medium heading-color" href={`/author/${news.authorName? news.authorName.toLowerCase() : ''}`}>
              {news.authorName}
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-16">
          {news.tags.split(',').map(tag => (
            <Link key={tag} className="text-xs font-semibold data-color rounded-full capitalize bg-gray-100 py-2 px-3 whitespace-nowrap" href={`/tag/${tag}`}>
              {tag}
            </Link>
          ))}
        </div>
